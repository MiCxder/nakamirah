...
export default function BeatCard({ beat }: { beat: Beat }) {
  ...
  const handlePlay = (e: React.MouseEvent) => {
    e.preventDefault(); // stop navigation when clicking play button
  ...
  return (
    <Link
      href={`/beats/${beat.id}`}
      className="block group relative rounded-2xl overflow-hidden border border-zinc-800 bg-zinc-900/40 backdrop-blur-xl hover:border-purple-500/40 transition-all duration-300 hover:scale-[1.02]"
    >
      {/* IMAGE */}
